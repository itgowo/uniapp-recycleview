<template>
	<view>
		<block v-if="items && items.length > minNum">
			<scroll-view scroll-y :style="{ minHeight: scrollviewMinHeight + 'px', height: scrollViewHeight, position: 'relative' }" @scroll="handleScroll">
		...
		<block v-else>
			<scroll-view scroll-y :style="{ height: scrollViewHeight + 'px' }">
				<view v-for="(item, index) in items" :key="index"><slot name="default" :item="item"></slot></view>
			</scroll-view>
		</block>
	</view>
</template>
...
export default {
	...
	props: {
		...
		scrollViewHeight: { type: String, default: '100vh' },
		visibleViewNum: {
			type: Number,
			default: 5
		},
